<!-- see https://github.com/antfu/talks/blob/main/2024-12-07/src/global-bottom.vue -->
 ...
 * - glowColors: [string, string, string] - Custom color for the polygons
 */
...
const customColors = computed(() => {
  return [
    formatter.value.glowColors?.[0] || 'from-[#2f96ad] to-white/10',
    formatter.value.glowColors?.[1] || 'from-[#4B32C3] to-white/10',
    formatter.value.glowColors?.[2] || 'from-[#8080f2] to-white/10',
  ]
})
...
<template>
  ...
  >
    <div
      :class="`clip bg-gradient-to-r ${customColors[0]}`"
      :style="{ 'clip-path': `polygon(${poly1})`, 'opacity': opacity }"
    />
    <div
      :class="`clip bg-gradient-to-l ${customColors[1]}`"
      :style="{ 'clip-path': `polygon(${poly2})`, 'opacity': opacity }"
    />
    <div
      :class="`clip bg-gradient-to-t ${customColors[2]}`"
